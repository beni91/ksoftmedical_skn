...
motif_rdv = [('suivi','Suivi'),('bilan','Bilan'),
             ('rdv','Rdv'),('urgence','Urgences'),
             ('autre','Autres')
]
...
class AppointmentAccount_Move(models.Model):
    ...
    libelle = fields.Selection([('consultation','Consultation'),('chirurgie','Chirurgie'),('laboratoire','Laboratoire'),
                                ('imagerie','Imagerie'),('pharmacie','Pharmacie'),('shop_optique','Shop Optique'),('hospi','Hospitalisation'),
                                ('urgence','Urgence'),('reanimation','Réanimation')], string="Libelle")
...
class ExamenLabo(models.Model):
    ...
    type_analyse = fields.Many2one(related='analyse.categ_id')
...
class Appointment(models.Model):
    ...
    ## Ophta
    type_consultation = fields.Selection([('stand', 'Standard'),('ophta', 'Ophtamologie'),('dent', 'Dentiste')], default='stand',string="Type de consultation")
        ...
        if traitement_ids or kine_ids:
            if traitement_ids:
                for ordo in traitement_ids:
                    lines.append((0, 0, {'date_traitement':fields.Date.today(), 
                                            'produit' : ordo.medicament.id or False, 
                                            'posologie' : ordo.description or "",
                                            'actes_medicaux' : ordo.product.id or False,
                                            'materiels' : ordo.materiels.id or False,
                                    }
                            ))

                hospi_id = self.env['module.hospitalisation'].create({
                    'patient_id': self.patient_id.id,
                    'medecin_tut': self.env.user.id,
                    'service_a': self.salle_soins.id,
                    'type_hospitalisation':'amb',
...
class FeuilleDeSurveillanceSVModule(models.Model):
    ...
    date_surveillance = fields.Date(string='Date', readonly=False, default=fields.Datetime.now())
