...
class WizardRapportConsultation(models.TransientModel):
    """ Permet de l'impression du rapport de consultation par spécialité. """

    _name = 'appointment.report'
    _description = 'Rapport de consultation'
    ...
    def get_consult_content(self):
        ...
        if self.start_date <= self.end_date:
            query = """
                    SELECT kservice.service, sum(moves3.amount_total_signed) as M_total, sum(moves1.amount_total_signed) as M_Payé, sum(moves3.amount_residual_signed) as M_Restant
                    ...
                    INNER JOIN (select service as service, id from ksoft_services) as kservice on kservice.id = moves3.t
                    ON moves1.ref=moves3.NN AND moves1.date BETWEEN '%s' AND '%s' AND moves3.payment_state IN ('paid', 'partial') AND moves1.journal_id=%s and moves3.libelle = 'consultation' and moves3.categ in ('prive', 'prive2')
          
                    GROUP BY kservice.service 
            ...
            cont += "<tfooter>"
            cont += "<tr>"
            cont += "<td style='text-align:right;'><b>TOTAL GEN.</b></td>"
            #cont += "<td style='text-align:center;background:#27AE60 ;color:#ffffff'><b>"+str(tmontant_attendu) +"</b></td>"
            cont += "<td style='text-align:center;background:#27AE60 ;color:#ffffff'><b>"+'{:,}'.format(tmontant_percu)+"</b></td>"
    ...
    def get_consultation_detail_convention(self):
        ...
        if self.start_date <= self.end_date:
            ...
            if (self.convention):
                ...
                query = """
                    SELECT moves1.invoice_date as D_facturation, moves1.date as D_Paiement,
                    moves1.invoice_partner_display_name as Patient,
                    moves1.convention_id as Convetion,
                    moves1.amount_total_signed as M_total, 
                    moves1.amount_total_signed as M_Payé, 
                    moves1.amount_residual_signed as M_Restant, 
                    moves1.payment_state as Etat, moves1.libelle as Libelle, 
                    moves1.matricule as Matricule, kservice.service as Services
                    FROM account_move as moves1 
                    INNER JOIN (select service as service, id as idk from ksoft_services) as kservice 
                    ON kservice.idk = moves1.service_id2 AND moves1.invoice_date BETWEEN '%s' AND '%s' AND moves1.payment_state IN ('paid', 'partial') 
                    AND moves1.libelle = 'consultation' AND moves1.categorie ='convention' AND moves1.state ='posted'
                    AND moves1.convention_id=%s order by moves1.Libelle asc
                ...
                for x in res:
                    ...
                    cont += "<td style='text-align:center;'>"+str(x[9]) +"</td>"
                    cont += "<td style='text-align:right;'>"+ '{:,}'.format((float(x[4]))) +"</td>"
                    # cont += "<td style='text-align:right;'>"+str(float(x[5])) +"</td>"
                    # cont += "<td style='text-align:right;'>"+str(float(x[6])) +"</td>"
                    # cont += "<td style='text-align:center;'>"+str(x[7]) +"</td>"
                    cont += "<td style='text-align:center;'>"+str(x[10]) +"</td>"
                ...
                cont += "<tr>"
                cont += "<td colspan='4' style='text-align:right;'><b>TOTAL GEN.</b></td>"
                cont += "<td style='text-align:right;background:#27AE60 ;color:#ffffff'><b>"+'{:,}'.format(tmontant_attendu) +"</b></td>"
                # cont += "<td style='text-align:right;background:#27AE60 ;color:#ffffff'><b>"+str(tmontant_percu)+"</b></td>"
                # cont += "<td style='text-align:right;background:#27AE60 ;color:#ffffff'><b>"+str(tmontant_restant) +"</b></td>"
                cont += "<td colspan='2' style='text-align:center;background:#27AE60 ;color:#ffffff'><b></b></td>"
                cont += "</tr>"
    ...
    def get_consultation_detail_prive(self):
        ...
        if self.start_date <= self.end_date:
            ...
            if (self.journal_payement and self.journal):
                ...
                query = """
                    SELECT moves3.invoice_date as D_facturation, moves1.date as D_Paiement, moves3.NN as N_Facture, moves3.invoice_partner_display_name as Patient, moves3.amount_total_signed as M_total, moves1.amount_total_signed as M_Payé, moves3.amount_residual_signed as M_Restant, 
                    moves3.payment_state as Etat, kservice.service, moves3.libelle as Libelle
                    FROM account_move as moves1 
                    INNER JOIN (SELECT moves2.categorie as categ, moves2.service_id2 as t, moves2.id, moves2.name as NN, moves2.date, moves2.ref, moves2.state, moves2.move_type, moves2.journal_id, moves2.partner_id, moves2.payment_reference, moves2.payment_id, moves2.amount_total_signed, moves2.amount_residual_signed, moves2.invoice_date,moves2.invoice_partner_display_name, moves2.payment_state,moves2.libelle FROM account_move as moves2) as moves3  
                    INNER JOIN (select service as service, id from ksoft_services) as kservice on kservice.id = moves3.t
                    ON moves1.ref=moves3.NN AND moves1.date BETWEEN '%s' AND '%s' AND moves3.payment_state IN ('paid', 'partial') AND moves1.journal_id=%s and moves3.libelle = 'consultation' and moves3.categ in ('prive', 'prive2') order by kservice.service asc
                ...
                for x in res:
                    ...
                    cont += "<td style='text-align:right;'>"+'{:,}'.format(float(x[6])) +"</td>"
                    cont += "<td style='text-align:center;'>"+str(x[7]) +"</td>"
                    cont += "<td style='text-align:center;'>"+str(x[8]) +"</td>"
                    cont += "<td style='text-align:center;'>"+str(x[9]) +"</td>"
                    cont += "</tr>"
                ...
                cont += "<td colspan='4' style='text-align:right;'><b>TOTAL GEN.</b></td>"
                cont += "<td style='text-align:right;background:#27AE60 ;color:#ffffff'><b>"+'{:,}'.format(tmontant_attendu) +"</b></td>"
                cont += "<td style='text-align:right;background:#27AE60 ;color:#ffffff'><b>"+'{:,}'.format(tmontant_percu)+"</b></td>"
                cont += "<td style='text-align:right;background:#27AE60 ;color:#ffffff'><b>"+'{:,}'.format(tmontant_restant) +"</b></td>"
